...
#include "../include/chunker_countsort_laszip.h"

#include "../include/Attributes.h"
#include "../include/converter_utils.h"
#include "../modules/unsuck/unsuck.hpp"
#include "../modules/unsuck/TaskPool.hpp"
#include "../include/Vector3.h"
#include "../include/ConcurrentWriter.h"

#include "../libs/json/json.hpp"
#include "../libs/laszip/laszip_api.h"
#include "../modules/LasLoader/LasLoader.h"
...
namespace chunker_countsort_laszip {
	...
	vector<std::atomic_int32_t> countPointsInCells(vector<Source> sources, Vector3 min, Vector3 max, int64_t gridSize, State& state, Attributes& outputAttributes) {
		...
		for (auto& v : grid) v.store(0, std::memory_order_relaxed);
		//任务结构体
		struct Task{
			string path;
			int64_t totalPoints = 0;
			int64_t firstPoint;//第一个点所在文件中的位置
			int64_t firstByte;
			int64_t numBytes;
			int64_t numPoints;
			int64_t bpp;
		...
		auto processor = [gridSize, &grid, tStart, &state, &outputAttributes](shared_ptr<Task> task){
			...
			for (int i = 0; i < numToRead; i++) {
				...
				if((start == 2158000227) && i == 0)
				{
					std::cout << "point: " << Vector3(coordinates[0],  coordinates[1], coordinates[2]).toString() << std::endl;
					std::cout << "first:"<<task->firstPoint<<std::endl;
					std::cout << "path:"<<path<<std::endl;
					std::cout << "bpp:"<<bpp<<std::endl;
					std::cout << "is_compressed:"<<is_compressed<<std::endl;
					std::cout << "numBytes:"<<numBytes<<std::endl;
					std::cout << "numToRead:"<<numToRead<<std::endl;
					std::cout <<".............................................."<<std::endl;
				}
				if((start == 2158000227) && i == 1)
				{
					std::cout << "point: " << Vector3(coordinates[0],  coordinates[1], coordinates[2]).toString() << std::endl;
				}
		...
		//for_each(parallel, paths.begin(), paths.end(), [&mtx, &sources](string path) {
			...
			//laszip_point* point;
			{
				...
				laszip_seek_point(laszip_reader, 83000000);
            	double coordinates[3];
            	laszip_read_point(laszip_reader);
            	laszip_get_coordinates(laszip_reader, coordinates);
            	std::cout << "test point: " << Vector3(coordinates[0],  coordinates[1], coordinates[2]).toString() << std::endl;
			}
			//每段点云数据的长度
	...
	NodeLUT createLUT(vector<atomic_int32_t>& grid, int64_t gridSize) {
		...
		for (auto& value : grid) {
			grid_high.push_back(value.load(std::memory_order_relaxed));
		}
		//获取gridSize的层级
		...
		for (int64_t level_low = level_max - 1; level_low >= 0; level_low--) {
			...
			for_xyz(gridSize_low, [&grid_low,  &grid_high, gridSize_low, gridSize_high, level_low, level_high, level_max](int64_t x, int64_t y, int64_t z){
				...
				//如果节点不需要合并同时节点内数量大于一定值时，对下一层不进行合并
	...
	void doChunking(vector<Source> sources, string targetDir, Vector3 min, Vector3 max, State& state, Attributes outputAttributes) {
		...
		// COUNT 计算每个格子里有多少个点(方法是确定每个点落在哪个格子里)
