...
#include "../include/HierarchyBuilder.h"
...
vector<NodeCandidate> createNodes(vector<vector<int64_t>>& pyramid) {
	...
	//根据pyramid中每个节点有多少个点来计算每个节点的偏移量
...
void doIndexing(string targetDir, State& state, Options& options, Sampler& sampler) {
	...
	//删除临时文件
	{
		...
		std::filesystem::path octreePath = std::filesystem::path(targetDir) / "tmpChunkRoots.bin";

		if (std::filesystem::exists(octreePath)) {
			try {
				std::filesystem::remove(octreePath);
				std::cout << "Removed: " << octreePath << std::endl;
			}
			...
			std::cout << "File does not exist: " << octreePath << std::endl;
		}
	}
