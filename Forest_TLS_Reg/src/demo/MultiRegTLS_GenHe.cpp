...
int main(int argc, char **argv) 
{
    ...
    std::string data_path2 = "/media/xiaochen/xch_disk2/genhe_TLS_data/Plot-007/007";
    std::cout << BOLDGREEN << "----------------DATA PROCESSING----------------" << RESET << std::endl;
    // read the setting parameters
    ConfigSetting config_setting;
    ReadParas(data_path+"/config/genhe_para.yaml", config_setting);
    // std::cout << "Here is good-1 " << std::endl; 
    // read the grund truth of pose data
    // std::vector<Eigen::Affine3d> tlsTrans = readTLSTrans(data_path+"/data/genhe/Transformation.txt");
    std::vector<Eigen::Affine3d> tlsTrans = readTLSTrans(data_path2+"/Transformation.txt");
    ...
    for(int i=1; i<=tlsTrans.size(); i++)
    {
        ...
        readTLSData(data_path2+ "/" + std::to_string(i) +".las", tls_point_data);
        // pcl::io::loadPLYFile(data_path+"/data/genhe/"+std::to_string(i)+".ply", *tls_point_data);
    ...
    //     cv::imwrite(data_path+"/data/genhe/" + std::to_string(i) + "-ori_fig.png", HashReg_RefTLS->pointMat_vec_[i]); 
    ...
    //         }
    //     }
    //     cv::imwrite(data_path+"/data/genhe/" + std::to_string(i) + "-filterd_fig.png", HashReg_RefTLS->pointMat_vec_[i]); 
    // }
    ...
    //     pcl::io::savePCDFile(data_path+"/data/genhe/" + std::to_string(i) + "-stem_position.pcd", *stem_position);
    //     pcl::io::savePCDFile(data_path+"/data/genhe/" + std::to_string(i) + "-stems.pcd", *objs);
    // }
    ...
    //     pcl::io::savePCDFile(data_path+"/data/genhe/" + std::to_string(i) + "-discard_cluster.pcd", *objs);
    // }
    ...
    write_error_vec(data_path+"/data/genhe/roughError.txt", rough_errors);
    write_pose(data_path+"/data/genhe/roughPoses.txt", tlsVec);
    ...
    write_error_vec(data_path+"/data/genhe/GTSAMError.txt", gtsam_errors);
    write_pose(data_path+"/data/genhe/GTSAMPoses.txt", optiTLSVec);
    ...
    for(int i=0; i<candidates_vec.size(); i++)
    {
        ...
        // // from las file
        // readTLSData(data_path+"/data/genhe/"+ std::to_string(currID+1) +".las", ICP_target); 
        ...
        *ICP_target = ds_points[currID];
        ...
        if(optiTLSVec[currID].isValued)
        {
            ...
            // writeLas(data_path+"/data/genhe/"+std::to_string(currID+1) +"-rough.las", ICP_target);
        ...
        for(int j=0; j<candidates_vec[i].candidateIDScore.size(); j++)
        {
            ...
            // // from las file
            // readTLSData(data_path+"/data/genhe/"+ std::to_string(candID+1) +".las", ICP_source);
            ...
            *ICP_source = ds_points[candID];
            ...
            if(optiTLSVec[candID].isValued)
            {
                ...
                // writeLas(data_path+"/data/genhe/"+std::to_string(candID+1) +"-rough.las", ICP_source);
    ...
    write_error_vec(data_path+"/data/genhe/icpError.txt", ICP_errors);
    write_pose(data_path+"/data/genhe/icpPoses.txt", optiPosVec);
    ...
    write_error_vec(data_path+"/data/genhe/finalError.txt", Final_errors);
    write_pose(data_path+"/data/genhe/finalPoses.txt", finalTLSVec);

    // write the running time for each module
    std::ofstream outfile(data_path+"/data/genhe/time_stastic.txt");
    ...
    std::ofstream timeFile(data_path+"/data/genhe/run_time.txt");
    timeFile << past.count();
    timeFile.close();

    // write the results
    std::cout << BOLDGREEN << "----------------Write the transformed data----------------" << RESET << std::endl;
    for(int i=0; i<optiPosVec.size(); i++)
    {
        std::cout << BOLDBLUE << "----------->" << optiPosVec[i].ID << " station!" << std::endl;
        if(optiPosVec[i].isValued)
        {
            Eigen::Matrix4d transform_i = Eigen::Matrix4d::Identity();
            transform_i.block<3,3>(0,0) = optiPosVec[i].R;
            transform_i.block<3,1>(0,3) = optiPosVec[i].t;
            
            // get origional data
            pcl::PointCloud<pcl::PointXYZ>::Ptr ori_ds_points(new pcl::PointCloud<pcl::PointXYZ>);
            *ori_ds_points = ds_points[optiPosVec[i].ID ];

            // trans point cloud
            pcl::PointCloud<pcl::PointXYZ>::Ptr out_ds_points(new pcl::PointCloud<pcl::PointXYZ>);
            pcl::transformPointCloud(*ori_ds_points, *out_ds_points, transform_i);
            
            std::string las_file = data_path2 + "/" + std::to_string(optiPosVec[i].ID+1) + "-transed.las";
            writeLas(las_file, out_ds_points);
        }
